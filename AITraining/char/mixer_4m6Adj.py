...
from util.config import CharConfig6Adj
from util.FlexDataRead import DataReadTrain, DataReadTest
from model.MLPmixer import MLPMixer

if not os.path.exists(CharConfig6Adj.SAVEDIR):
    os.makedirs(CharConfig6Adj.SAVEDIR)

logfile = '%s/trainDigit.log' % CharConfig6Adj.SAVEDIR
trainlog(logfile)

data_set = {}
data_set['train']=DataReadTrain(r"../../../data/flexData/chars/char1","w",sixPointAdj)
data_set['val']=DataReadTest(r"../../../data/flexData/chars/charFlex/26char","w",sixPointAdj)
# data_set['test']=FlexSensorDataRead(r"../../data/temp/picFlex/digitGen/test","digit",fivePoint)


dataloader={}
dataloader['train'] = DataLoader(dataset=data_set['train'], batch_size=CharConfig6Adj.BATCHSIZE, shuffle=True)
dataloader['val'] = DataLoader(dataset=data_set['val'], batch_size=CharConfig6Adj.BATCHSIZE, shuffle=True)



mlp_mixer = MLPMixer(in_channels=6, image_size=6, num_patch=6, num_classes=CharConfig6Adj.NUM_CLASS,
...
mlp_mixer.load_state_dict(torch.load("./output/CharConfig6Adj/weights-29-13587-[0.6422].pth"))

mlp_mixer = mlp_mixer.cuda()

criterion = nn.CrossEntropyLoss()
#optimizer = optim.SGD(model.parameters(), lr=opt.learning_rate, momentum=0.9, weight_decay=1e-5)
optimizer = optim.Adam(mlp_mixer.parameters(), lr=CharConfig6Adj.LR)

#scheduler = StepLR(optimizer, step_size=1, gamma=CharConfig6Adj.GAMA)
...
# train(mlp_mixer,
#           epoch_num=CharConfig6Adj.EPOCHES,
#           start_epoch=CharConfig6Adj.START_EPOCHES,
#           optimizer=optimizer,
#           criterion=criterion,
#           exp_lr_scheduler=scheduler,
#           data_set=data_set,
#           data_loader=dataloader,
#           save_dir=CharConfig6Adj.SAVEDIR,
#           print_inter=CharConfig6Adj.PRINT_INTERVAL,
#           val_inter=CharConfig6Adj.SAVS_INTERVAL)

# save_path = os.path.join(CharConfig6Adj.SAVEDIR,
...
plt.savefig('/home/iot/jupyter/root_dir/liudongdong/src/ai/char/output/confusion/{}.png'.format('mixer_4m6Adj'), format='png')


#nohup python mixer_4m6Adj.py >>../output/char/4m6pAdj.out