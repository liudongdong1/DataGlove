...
mlp_mixer = MLPMixer(in_channels=5, image_size=5, num_patch=5, num_classes=CharConfig5Point.NUM_CLASS,
                     dim=128, depth=1, token_dim=256, channel_dim=1024)
parameters = filter(lambda p: p.requires_grad, mlp_mixer.parameters())
parameters = sum([np.prod(p.size()) for p in parameters]) / 1_000_000
print('Trainable Parameters: %.3fM' % parameters)

mlp_mixer.load_state_dict(torch.load('./output/CharConfig5Point/weights-14-23122-[0.7960].pth'))
...
def plot_Matrix(cm, classes, title=None,  cmap=plt.cm.Blues):
    ...
    fig.tight_layout()
    plt.savefig('./mixer_1m5pV2.png', dpi=300)
    plt.show()

cm = confusion_matrix(ALLlabels,ALLpreds)
ALLpreds=np.column_stack((ALLlabels,ALLpreds))
np.savetxt("1m5p.txt", ALLpreds,fmt='%d',delimiter=',')
#print(cm)
labels_name={'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z'}
plot_confusion_matrix(cm, labels_name,"")
plt.savefig('/home/iot/jupyter/root_dir/liudongdong/src/ai/char/output/confusion/{}.png'.format('mixer_1m5p'), format='png')
