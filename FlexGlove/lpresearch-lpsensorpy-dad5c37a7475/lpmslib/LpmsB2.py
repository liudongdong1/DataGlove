...
class LpmsB2(object):
    ...
    def __parse(self, data):
        self.lrcReceived = 0
        #print(type(data),data)  #<class 'bytes'> b':\x01\x00\t\x00P\x00\xc10\x00\x00\xb7{j
        for c in data:
            #print(type(c),c)
            #b = ord(c)
            b=c
    ...
    def __send_data(self, function, length):
        ...
        if length > 0:
            ...
            txlrc_check += sum([c for c in self.raw_tx_data])
    ...
    def stop_sync(self):
        if not self.is_connected():
            loge(self.TAG, "sensor not connected")
            return None
        if self.verbose: logd(self.TAG, "Stop Sync")
    ...
    def get_debug_log(self, indexStart, indexStop):
        ...
        # to download debug_log including indexStop
