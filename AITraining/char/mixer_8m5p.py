...
from util.config import CharConfig5Point
from util.FlexDataRead import DataReadTrain, DataReadTest
from model.MLPmixer import MLPMixer
from torch.utils.mobile_optimizer import optimize_for_mobile
if not os.path.exists(CharConfig5Point.SAVEDIR):
    os.makedirs(CharConfig5Point.SAVEDIR)

logfile = '%s/trainDigit.log' % CharConfig5Point.SAVEDIR
trainlog(logfile)

data_set = {}
data_set['train']=DataReadTrain(r"../../../data/flexData/chars/char1","w",fivePoint)
data_set['val']=DataReadTest(r"../../../data/flexData/chars/charFlex/26char","w",fivePoint)
# data_set['test']=FlexSensorDataRead(r"../../data/temp/picFlex/digitGen/test","digit",fivePoint)

dataloader={}
dataloader['train'] = DataLoader(dataset=data_set['train'], batch_size=CharConfig5Point.BATCHSIZE, shuffle=True)
dataloader['val'] = DataLoader(dataset=data_set['val'], batch_size=CharConfig5Point.BATCHSIZE, shuffle=True)
...
mlp_mixer = MLPMixer(in_channels=5, image_size=5, num_patch=5, num_classes=CharConfig5Point.NUM_CLASS,
                     dim=128, depth=8, token_dim=256, channel_dim=1024)
parameters = filter(lambda p: p.requires_grad, mlp_mixer.parameters())
parameters = sum([np.prod(p.size()) for p in parameters]) / 1_000_000
print('Trainable Parameters: %.3fM' % parameters)

mlp_mixer.load_state_dict(torch.load('/home/iot/jupyter/root_dir/liudongdong/src/ai/char/output/CharConfig5Point/weights-5-5483-[0.8062].pth'))
...
#optimized_traced_model._save_for_lite_interpreter("/home/iot/jupyter/root_dir/liudongdong/src/ai/digit/output/DigitConfig5Point/modeltoandroidchar_jit.pt")
## 添加部分cqx,2021.11.29

criterion = nn.CrossEntropyLoss()
#optimizer = optim.SGD(model.parameters(), lr=opt.learning_rate, momentum=0.9, weight_decay=1e-5)
optimizer = optim.Adam(mlp_mixer.parameters(), lr=CharConfig5Point.LR)

#scheduler = StepLR(optimizer, step_size=1, gamma=CharConfig5Point.GAMA)
...
#----------------- 训练代码部分 ----------------
# train(mlp_mixer,
#           epoch_num=CharConfig5Point.EPOCHES,
#           start_epoch=CharConfig5Point.START_EPOCHES,
#           optimizer=optimizer,
#           criterion=criterion,
#           exp_lr_scheduler=scheduler,
#           data_set=data_set,
#           data_loader=dataloader,
#           save_dir=CharConfig5Point.SAVEDIR,
#           print_inter=CharConfig5Point.PRINT_INTERVAL,
#           val_inter=CharConfig5Point.SAVS_INTERVAL)

# save_path = os.path.join(CharConfig5Point.SAVS_INTERVAL,
#                         'finaldigitTrans2m.pth')
...
def plot_Matrix(cm, classes, title=None,  cmap=plt.cm.Blues):
    ...
    fig.tight_layout()
    plt.savefig('./mixer_8m5pV2.png', dpi=300)
    plt.show()

cm = confusion_matrix(ALLlabels,ALLpreds)
ALLpreds=np.column_stack((ALLlabels,ALLpreds))
np.savetxt("8m5p.txt", ALLpreds,fmt='%d',delimiter=',')
#print(cm)
labels_name={'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z'}
plot_confusion_matrix(cm, labels_name,"")
plt.savefig('/home/iot/jupyter/root_dir/liudongdong/src/ai/char/output/confusion/{}.png'.format('mixer_8m5p'), format='png')
