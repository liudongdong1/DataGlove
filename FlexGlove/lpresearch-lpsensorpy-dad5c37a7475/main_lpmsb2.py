...
def exec_menu(menu, choice):
    ...
    if printer_running:
        ...
        if ch == '':
            lputils.loge(TAG, "Empty input, please try again.")
        else:
            try:
                menu[ch]()
            except KeyError:
...
def get_debug_log_size():
    logSize = lpmsb.get_debug_log_size()
    print (logSize)

def get_debug_log():
    print ("getting debug log from sensor")
    start = int(raw_input(" >> Start debug_log_index: "))
    stop = int(raw_input(" >> Stop debug_log_index: "))
    lpmsb.get_debug_log(start, stop)
